...
class MainView(discord.ui.View):
    ...
    async def view(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(
            name="Solos",
            value=f"Tank: {', '.join(solos['tank']) or '—'}\n"
                  f"Heal: {', '.join(solos['heal']) or '—'}\n"
                  f"DPS: {', '.join(solos['dps']) or '—'}",
            inline=False
        )
...
class SoloModal(discord.ui.Modal, title="Inscription Solo"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if role not in solos:
            await interaction.response.send_message("Role invalide", ephemeral=True)
            return

        solos[role].append(interaction.user.name)
